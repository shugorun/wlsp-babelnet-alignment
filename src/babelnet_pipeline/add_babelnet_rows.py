#!/usr/bin/env python
# Usage: python src/babelnet_pipeline/add_babelnet_rows.py --input-path data/gold/gold_A_records.parquet --babelnet-path data/processed/babelnet_.parquet
...
DEFAULT_BABELNET_PATH = ROOT / "data" / "processed" / "babelnet_.parquet"
MAX_WORKERS = 4
...
def append_missing_babelnet_rows(
    input_path: Path,
    babelnet_path: Path,
    max_workers: int,
    include_hypernyms: bool,
) -> None:
...
def main() -> None:
    ...
    ap.add_argument("--babelnet-path", type=Path, default=DEFAULT_BABELNET_PATH)
    ap.add_argument("--max-workers", type=int, default=MAX_WORKERS)
    ap.add_argument("--no-hypernyms", action="store_true")
    args = ap.parse_args()

    append_missing_babelnet_rows(
        input_path=args.input_path,
        babelnet_path=args.babelnet_path,
        max_workers=args.max_workers,
        include_hypernyms=not args.no_hypernyms,
    )
